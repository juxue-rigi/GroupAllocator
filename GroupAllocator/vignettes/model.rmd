...
df_pref_expanded <- df_pref_expanded %>%
  mutate(preference_score = if_else(pref_rank == 1, 100,
                             if_else(pref_rank == 2, 80,
                             if_else(pref_rank == 3, 60,
                                     40))))
...
model <- MIPModel() %>%
  ...
  # 3) Objective: maximize sum of preferences - penalty * total slack
  set_objective(
  ...
  # 4a) Each group assigned exactly once
  add_constraint(
    sum_expr(A[g, t, j, s],
  ...
  # 4b) Each sub-team must have at least 1 group => sub-team is not "skipped"
  add_constraint(
    sum_expr(A[g, t, j, s], g = 1:n_groups) >= 1,
    t = 1:n_topics, j = 1:x_topic_teams, s = 1:n_subteams
  ) %>%
  
  # 4c) Sub-team capacity with slack
  # sum of group sizes assigned + slack = b_subteam
  add_constraint(
